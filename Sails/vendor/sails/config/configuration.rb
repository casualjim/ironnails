...
module Sails
  ...
  class Configuration
    ...
    # Set the root_path to SAILS_ROOT and canonicalize it.
    def set_root_path!
      raise 'SAILS_ROOT is not set' unless defined?(::SAILS_ROOT)
      raise 'SAILS_ROOT is not a directory' unless File.directory?(::SAILS_ROOT)
      
      @root_path =
      # Pathname is incompatible with Windows, but Windows doesn't have
      # real symlinks so File.expand_path is safe.
      if RUBY_PLATFORM =~ /(:?mswin|mingw)/
        File.expand_path(::SAILS_ROOT)
        
        # Otherwise use Pathname#realpath which respects symlinks.
      else
        Pathname.new(::SAILS_ROOT).realpath.to_s
      end
      
      Object.const_set(:RELATIVE_SAILS_ROOT, ::SAILS_ROOT.dup) unless defined?(::RELATIVE_SYLVESTER_ROOT)
      
      ::SAILS_ROOT.replace @root_path
    end
